            ...
            if original_depth is not None:
                ...
                for i, patch in enumerate(arr):
                    ...
                    if i == 0:
                        ...
                        # 計算與前一個 patch 嘅重疊大小（固定為 overlap，除非剩餘深度少過）
                        overlap_size = min(overlap, actual_depth)
                        ...
                        if overlap_size > 0:
                            ...
                            # 非重疊部分
                            non_overlap_start = overlap_size
                            non_overlap_end = actual_depth
                            if non_overlap_start < non_overlap_end:
                                arr_result[:, :, overlap_end:end_idx] = patch[:, :, non_overlap_start:non_overlap_end]
                        else:
                            # 冇重疊，直接複製
                            arr_result[:, :, start_idx:end_idx] = patch[:, :, :actual_depth]
